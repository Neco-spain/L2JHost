...
public class AioMenu implements IUserCommandHandler
{
	...
	public static void bypass(Player activeChar, String command, StringTokenizer st)
	{
		...
		{
			final List<TeleportLocation> list = TeleportData.getInstance().getTeleports(Integer.parseInt(st.nextToken()));
			if (list != null)
			{
				TeleportLocation location = list.get(0); // Pega o primeiro TeleportLocation da lista
				if (activeChar.reduceAdena("Teleport", location.getPriceCount(), activeChar, true))
					activeChar.teleportTo(location, 0);
			}
			else
