...
public class AdminSpawn implements IAdminCommandHandler
{
	...
	public void useAdminCommand(String command, Player player)
	{
		...
		else if (command.startsWith("admin_delete_xml"))
		{
			// Target must be a Npc.
			final WorldObject targetWorldObject = player.getTarget();
			if (!(targetWorldObject instanceof Npc))
			...
			// Generating the XML filename based on the NPC's properties.
			String npcName = targetNpc.getName().toLowerCase();
			int identifier1 = targetNpc.getX();
			int identifier2 = targetNpc.getY();
			String xmlFileName = "./data/xml/spawnlist/custom" + npcName + "_" + identifier1 + "_" + identifier2 + ".xml";
			...
			if (xmlFile.exists())
			{
				try
				{
					if (xmlFile.delete())
					{
						player.sendMessage("Deleted XML file for " + targetNpc.getName() + ".");
					}
					else
					{
						player.sendMessage("Failed to delete XML file for " + targetNpc.getName() + ".");
						LOGGER.warn("Failed to delete XML file: " + xmlFileName);
					}
				}
				catch (Exception e)
				{
					player.sendMessage("Error occurred while deleting XML file for " + targetNpc.getName() + ".");
					LOGGER.error("Error deleting XML file: " + xmlFileName, e);
				}
			}
			else
			{
				player.sendMessage("XML file for " + targetNpc.getName() + " not found.");
				LOGGER.warn("XML file not found: " + xmlFileName);
			}
		}
