...
public class Mdam implements ISkillHandler
{
	...
	public void useSkill(Creature activeChar, L2Skill skill, WorldObject[] targets, ItemInstance itemInstance)
	{
		...
		for (WorldObject obj : targets)
		{
			...
			{
				if (activeChar instanceof Playable && target instanceof Playable && activeChar != target)
				{
					Player at = (Player) activeChar;
					Player t = (Player) target;
					
					double classToClassMod = ((BalanceManagerSkillAI.getInstance().getPowerMod(at.getClassId().getId(), t.getClassId().getId()) / 100f) * damage);
					double classToAllMod = ((BalanceManagerSkillAI.getInstance().getPowerMod(at.getClassId().getId(), -1) / 100f) * damage);
					damage += classToClassMod + classToAllMod;
					
					if (Config.ENABLE_BALANCE_LOGGER_MAGICAL)
					{
