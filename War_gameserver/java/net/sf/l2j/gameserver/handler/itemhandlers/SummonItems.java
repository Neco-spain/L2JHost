...
{
	public static boolean _unsumon;
	
	public static int _npcId;
	
	public static int setNpcId(int id)
	{
		return _npcId = id;
	}
		...
		switch (sitem.getValue())
		{
			...
			case 3: // Agnation
				...
				else
				{
					...
					player.getAI().tryToCast(player, skill, false, false, (0));
					
					player.getMove().stop();
					setNpcId(npcTemplate.getNpcId());
					ThreadPool.schedule(() -> SpawnAgathion(player), skill.getHitTime());
				}
	...
	public void SpawnAgathion(Player player)
	{
		final NpcTemplate npcTemplate = NpcData.getInstance().getTemplate(getNpcId());
		...
		try
		{
			...
			npc.getAI().setFollowStatus(true);
			
			AgathionAI seletec = npc.AgathionAI();
			seletec.start(player);
			
			player.setPlayerAgathion(true);
			setNpcId(0);
		}
		catch (Exception e)
		{
