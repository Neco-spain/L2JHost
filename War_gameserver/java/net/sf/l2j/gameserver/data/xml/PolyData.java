...
public class PolyData implements IXmlReader
{
	private final Map<Integer, PolyPc> _fakePcs = new HashMap<>();
	...
	public void load()
	{
		parseFile("./data/xml/poly.xml");
		LOGGER.info("Loaded {} Poly players data.", _fakePcs.size());
	}
	...
	public void parseDocument(Document doc, Path path)
	{
		forEach(doc, "list", listNode -> forEach(listNode, "npc", node ->
		{
			...
			_fakePcs.put(set.getInteger("id"), new PolyPc(set));
		}));
	}
	...
	public PolyPc getFakePc(int npcId)
	{
		return _fakePcs.get(npcId);
	}
	
	public static PolyData getInstance()
	{
	...
	{
		protected static final PolyData _instance = new PolyData();
	}
}