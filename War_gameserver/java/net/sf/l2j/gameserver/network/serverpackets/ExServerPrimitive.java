...
public class ExServerPrimitive extends L2GameServerPacket
{
	...
	public void addCircle(String name, Color color, boolean isNameColored, int x, int y, int z, int radius)
	{
		int numPoints = 36; //box
		double angleIncrement = (2 * Math.PI) / numPoints;
		
		for (int i = 0; i < numPoints; i++)
		{
			double angle = i * angleIncrement;
			int pointX = x + (int) (radius * Math.cos(angle));
			int pointY = y + (int) (radius * Math.sin(angle));
			
			addPoint(name, color, isNameColored, pointX, pointY, z);
		}
	}
