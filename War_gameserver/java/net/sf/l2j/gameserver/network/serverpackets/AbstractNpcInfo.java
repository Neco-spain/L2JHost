...
import net.sf.l2j.gameserver.data.xml.PolyData;
import net.sf.l2j.gameserver.data.xml.PolyData.PolyPc;
...
public abstract class AbstractNpcInfo extends L2GameServerPacket
{
	...
	public static class NpcInfo extends AbstractNpcInfo
	{
		...
		protected void writeImpl()
		{
			...
			PolyPc fpc = PolyData.getInstance().getFakePc(_npc.getNpcId());
			if (fpc != null)
			{
				writeC(0x03);
				writeD(_x);
				...
				writeS(fpc.getName()); // visible name
				writeD(fpc.getRace());
				writeD(fpc.getSex());
				...
				writeD(0x00); //
				writeD(fpc.getRightHand()); // PAPERDOLL_RHAND
				writeD(fpc.getLeftHand()); // PAPERDOLL_LHAND
				writeD(fpc.getGloves()); // PAPERDOLL_GLOVES
				writeD(fpc.getChest()); // PAPERDOLL_CHEST
				writeD(fpc.getLegs()); // PAPERDOLL_LEGS
				writeD(fpc.getFeet()); // PAPERDOLL_FEET
				writeD(fpc.getHair()); // PAPERDOLL_HAIR
				writeD(fpc.getRightHand()); // PAPERDOLL_RHAND
				writeD(fpc.getHair()); // PAPERDOLL_HAIR
				writeD(fpc.getHair2()); // PAPERDOLL_HAIR2
				...
				writeD(_runSpd);
				writeD(_walkSpd);
				writeF(_npc.getStatus().getMovementSpeedMultiplier()); // _activeChar.getProperMultiplier()
				writeF(_npc.getStatus().getAttackSpeedMultiplier()); // _activeChar.getAttackSpeedMultiplier()
				...
				if (_npc instanceof Monster)
					writeS(fpc.getTitle() + " - HP " + new DecimalFormat("#.##").format(100.0 * _npc.getStatus().getHp() / _npc.getStatus().getMaxHp()) + "%"); // visible titl
				else
					writeS(fpc.getTitle());
				
				writeD(fpc.getClanId()); // clan id
				writeD(fpc.getClanCrest()); // clan crest id
				writeD(fpc.getAllyId()); // ally id
				writeD(fpc.getAllyCrest()); // ally crest id
	...
	void write(char type, int value, int times)
	{
