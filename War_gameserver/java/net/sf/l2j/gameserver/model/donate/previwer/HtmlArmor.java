...
public class HtmlArmor
{
	private static final int ARMOR_PER_PAGE = 7;
	
	public static void listArmor(Player player, int page)
	{
		final NpcHtmlMessage content = new NpcHtmlMessage(0);
		content.setFile(player.isLang() + "mods/donate/ListArmor.htm");
		boolean isTableGenerated = false;
		
		final Pagination<ArmorSet> list = new Pagination<>(DataArmorSet.getInstance().getSets().stream(), page, ARMOR_PER_PAGE);
		...
		for (ArmorSet armorSet : list)
		{
			...
			String itemName = armorSet.toString().length() > 22 ? armorSet.toString().substring(0, 22) + "..." : armorSet.toString();
			...
			int setItemId = armorSet.getSetItemsId()[0];
			sb.append("<td height=\"40\" width=\"40\"><button action=\"bypass donate create_armor " + player.getName() + " " + setItemId + " " + page + "\" width=32 height=32 back=\"exclusive.LCoinShopWnd_Buy_Button_Over\" fore=\"exclusive.LCoinShopWnd_Buy_Button\"></td>");
		...
		sb.append("<table width=\"300\" height=\"15\" bgcolor=\"000000\"><tr>");
		if (page > 1)
			sb.append("<td align=left width=70><a action=\"bypass donate list_set " + String.valueOf(page - 1) + "\">Previous</a></td>");
		else
			sb.append("<td align=left width=70>Previous</td>");
		
		sb.append("<td align=center width=128> Page: " + page + "</td>");
		if (list.size() > page)
			sb.append("<td align=right width=70><a action=\"bypass donate list_set " + String.valueOf(page + 1) + "\">Next</a></td>");
		else
	...
	public static void handleCreateArmorEquipped(Player player, Player target, int chestId, int page)
	{
		try
		{
			final ArmorSet armorSet = DataArmorSet.getInstance().getSet(chestId);
			...
			if (armorSet.getShield() > 0)
			{
				...
				ItemInstance item = target.addItem("AuctionPurchase", armorSet.getShield(), 1, target, true);
				item.setEnchantLevel(armorSet.getEnchantLevel());
				target.useEquippableItem(item, true);
				InventoryUpdate playerIU = new InventoryUpdate();
				playerIU.addItem(item);
			...
			target.sendPacket(new ExShowScreenMessage(target.getName() + " You have successfully purchase item. " + armorSet.toString(), (int) TimeUnit.SECONDS.toMillis(5)));
			
			listArmor(player, page);
