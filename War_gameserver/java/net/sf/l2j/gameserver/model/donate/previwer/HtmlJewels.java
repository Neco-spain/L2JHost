...
public class HtmlJewels
{
	private static final int ARMOR_PER_PAGE = 7;
	
	public static void listJewels(Player player, int page)
	{
		final NpcHtmlMessage content = new NpcHtmlMessage(0);
		content.setFile(player.isLang() + "mods/donate/ListJewels.htm");
		boolean isTableGenerated = false;
		
		final Pagination<JewelSet> list = new Pagination<>(JewelSetData.getInstance().getSets().stream(), page, ARMOR_PER_PAGE);
		
		final StringBuilder sb = new StringBuilder();
		int row = 0;
		int count = 0;
		for (JewelSet armorSet : list)
		{
			if (!isTableGenerated)
			{
			...
			sb.append("<td height=\"40\" width=\"40\"><button action=\"bypass donate create_jewels " + player.getName() + " " + setItemId + " " + page + "\" width=32 height=32 back=\"exclusive.LCoinShopWnd_Buy_Button_Over\" fore=\"exclusive.LCoinShopWnd_Buy_Button\"></td>");
		...
		sb.append("<table width=\"300\" height=\"15\" bgcolor=\"000000\"><tr>");
		if (page > 1)
			sb.append("<td align=left width=70><a action=\"bypass donate list_jewels " + String.valueOf(page - 1) + "\">Previous</a></td>");
		else
			sb.append("<td align=left width=70>Previous</td>");
		
		sb.append("<td align=center width=128> Page: " + page + "</td>");
		if (list.size() > page)
			sb.append("<td align=right width=70><a action=\"bypass donate list_jewels " + String.valueOf(page + 1) + "\">Next</a></td>");
		else
	...
	public static void handleCreateJewelsEquipped(Player player, Player target, int chestId, int page)
	{
		try
		{
			final JewelSet armorSet = JewelSetData.getInstance().getSet(chestId);
