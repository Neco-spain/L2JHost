...
/**
 * This container contains all informations concerning an item (weapon, armor,
 * etc).
 */
public abstract class Item {
	...
	/**
	 * @param enchantLevel
	 * @return int the quantity of crystals for crystallization on specific enchant
	 *         level
	 */
	...
	public boolean checkCondition(Creature creature, WorldObject object, boolean sendMessage) {
		...
		if ((isOlyRestrictedItem() || isHeroItem())
				&& (creature instanceof Player && creature.getActingPlayer().isInOlympiadMode())) {
			if (isEquipable())
	...
	public boolean isType(int type1, int type2) {
		return (this._type1 == type1) && (this._type2 == type2);
	}
