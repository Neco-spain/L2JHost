...
public class VillageMaster extends Folk {
		...
		if (actualCommand.equalsIgnoreCase("create_clan")) {
			...
			for (Castle castle : CastleManager.getInstance().getCastles()) {
				if (castle.getSiege().checkSides(clan)) {
					player.sendPacket(
							(castle.getSiege().isInProgress()) ? SystemMessageId.CANNOT_DISSOLVE_WHILE_IN_SIEGE
			...
			if (member == null) {
				player.sendPacket(
						SystemMessage.getSystemMessage(SystemMessageId.S1_DOES_NOT_EXIST).addString(cmdParams));
				return;
			}
			...
			// Affecting subclasses (add/del/change) if registered in Olympiads makes you
			// ineligible to compete.
			if (OlympiadManager.getInstance().isRegisteredInComp(player))
				OlympiadManager.getInstance().unRegisterNoble(player);

			if (player.isRegisteredInTournament()) {
				player.sendMessage("You can not change sub class while registered at Tournament.");
				return;
			}
			...
			case 1: // Add Subclass - Initial
				...
				// Avoid giving player an option to add a new sub class, if they have three
				// already.
				...
				for (ClassId subClass : subsAvailable)
					StringUtil.append(sb, "<a action=\"bypass -h npc_%objectId%_Subclass 4 ", subClass.getId(),
							"\" msg=\"1268;", subClass, "\">", subClass, "</a><br>");
			...
			case 2: // Change Class - Initial
				...
				if (player.getSubClasses().isEmpty())
					...
					if (checkVillageMaster(player.getBaseClass()))
						StringUtil.append(sb, "<a action=\"bypass -h npc_%objectId%_Subclass 5 0\">",
								PlayerData.getInstance().getClassNameById(player.getBaseClass()), "</a><br>");

					for (SubClass subclass : player.getSubClasses().values()) {
						if (checkVillageMaster(subclass.getClassDefinition()))
							StringUtil.append(sb, "<a action=\"bypass -h npc_%objectId%_Subclass 5 ",
									subclass.getClassIndex(), "\">", subclass.getClassDefinition(), "</a><br>");
					}
			...
			case 3: // Change/Cancel Subclass - Initial
				...
				if (player.getSubClasses().containsKey(1))
					html.replace("%sub1%",
							PlayerData.getInstance().getClassNameById(player.getSubClasses().get(1).getClassId()));
				else
					html.replace("<a action=\"bypass -h npc_%objectId%_Subclass 6 1\">%sub1%</a><br>", "");

				if (player.getSubClasses().containsKey(2))
					html.replace("%sub2%",
							PlayerData.getInstance().getClassNameById(player.getSubClasses().get(2).getClassId()));
				else
					html.replace("<a action=\"bypass -h npc_%objectId%_Subclass 6 2\">%sub2%</a><br>", "");

				if (player.getSubClasses().containsKey(3))
					html.replace("%sub3%",
							PlayerData.getInstance().getClassNameById(player.getSubClasses().get(3).getClassId()));
				else
			...
			case 4: // Add Subclass - Action (Subclass 4 x[x])
				...
				// Verify if the character has completed the Mimir's Elixir (Path to Subclass)
				// and Fate's Whisper (A Grade Weapon) quests by checking for instances of their
				// unique reward items.
			...
			case 6: // Change/Cancel Subclass - Choice
				...
				sb = new StringBuilder(300);
				for (ClassId subClass : subsAvailable)
					StringUtil.append(sb, "<a action=\"bypass -h npc_%objectId%_Subclass 7 ", paramOne, " ",
							subClass.getId(), "\" msg=\"1445;", "\">", subClass, "</a><br>");
			...
			// this class dont know any other commands, let forward the command to the
			// parent class
	...
	/**
	 * Check subclass classId for validity (villagemaster race/type is not contains
	 * in previous subclasses, but in allowed subclasses). Base class not added into
	 * allowed subclasses.
	 * 
	...
	private static final void createSubPledge(Player player, String clanName, String leaderName, int pledgeType,
			int minClanLvl) {
		if (!player.isClanLeader()) {
			player.sendPacket(SystemMessageId.YOU_ARE_NOT_AUTHORIZED_TO_DO_THAT);
			return;
		...
		for (Clan tempClan : ClanTable.getInstance().getClans()) {
			if (tempClan.getSubPledge(clanName) != null) {
				if (pledgeType == Clan.SUBUNIT_ACADEMY)
					player.sendPacket(
							SystemMessage.getSystemMessage(SystemMessageId.S1_ALREADY_EXISTS).addString(clanName));
				else
	...
	private static final void assignSubPledgeLeader(Player player, String clanName, String leaderName) {
		...
		clan.broadcastToMembers(new PledgeShowMemberListAll(clan, subPledge.getId()),
				SystemMessage.getSystemMessage(SystemMessageId.S1_HAS_BEEN_SELECTED_AS_CAPTAIN_OF_S2)
						.addString(leaderName).addString(clanName));
	}
