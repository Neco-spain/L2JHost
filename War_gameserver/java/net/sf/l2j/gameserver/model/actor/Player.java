...
import net.sf.l2j.gameserver.model.entity.events.tournament.TournamentConfig;
import net.sf.l2j.gameserver.model.entity.events.tournament.TournamentState;
...
public final class Player extends Playable {
	...
	final Location _savedLocation = new Location(0, 0, 0);
	...
	/**
	 * A newbie is a {@link Player} between level 6 and 25 which didn't yet acquired
	 * first occupation change.<br>
	 * <br>
	 * <b>Since IL, Newbie statut isn't anymore the first character of an account
	 * reaching that state, but any.</b>
	 * 
	...
	public boolean isNewbie(boolean checkLowLevel) {
		return (checkLowLevel)
				? (getClassId().getLevel() <= 1 && getStatus().getLevel() >= 6 && getStatus().getLevel() <= 25)
				: (getClassId().getLevel() <= 1 && getStatus().getLevel() <= 25);
	}
	...
	/**
	 * @return True if the state of {@link OperateType} of this {@link Player} is
	 *         different than NONE.
	 */
	@Override
	...
	public int getWeightLimit() {
		return (int) getStatus().calcStat(Stats.WEIGHT_LIMIT,
				69000 * Formulas.CON_BONUS[getStatus().getCON()] * Config.WEIGHT_LIMIT, this, null);
	}
	...
	public void refreshWeightPenalty() {
		...
		final double ratio = (getCurrentWeight() - getStatus().calcStat(Stats.WEIGHT_PENALTY, 0, this, null))
				/ weightLimit;
	...
	public void useEquippableItem(ItemInstance item, boolean abortAttack) {
		...
		if (isEquipped) {
			...
			if (item.getItemType() == WeaponType.BOW && getActingPlayer().getAttackType().equals(WeaponType.BOW)) {
				final L2Skill skill = SkillTable.getInstance().getInfo(313, SkillTable.getInstance().getMaxLevel(313));
				getActingPlayer().stopSkillEffects(skill.getId());
			}
	...
	public void setClassId(int id) {
		...
		try {
			if (getLvlJoinedAcademy() != 0 && _clan != null && ClassId.VALUES[id].getLevel() == 2) {
				...
				_clan.broadcastToMembers(SystemMessage.getSystemMessage(
						SystemMessageId.CLAN_ACADEMY_MEMBER_S1_HAS_SUCCESSFULLY_COMPLETED_THE_2ND_CLASS_TRANSFER_AND_OBTAINED_S2_CLAN_REPUTATION_POINTS)
						.addString(getName()).addNumber(points));
				...
				_clan.broadcastToMembersExcept(this, new PledgeShowMemberListDelete(getName()), SystemMessage
						.getSystemMessage(SystemMessageId.S1_HAS_WITHDRAWN_FROM_THE_CLAN).addString(getName()));
	...
	public ItemInstance addItem(String process, int itemId, int count, WorldObject reference, boolean sendMessage) {
		if (count > 0) {
			...
			// Sends message to client if requested. Since Herbs are directly assimilated,
			// they don't send any "You have earned X" message, only "The effects of X flow
			// through you".
			if (sendMessage && item.getItemType() != EtcItemType.HERB) {
				if (count > 1) {
					if (process.equalsIgnoreCase("Sweep") || process.equalsIgnoreCase("Harvest")
	...
	public void onAction(Player player, boolean isCtrlPressed, boolean isShiftPressed) {
		...
		ExclusiveBot bot = this.getBot();

		ExclusiveBot bottarget = player.getBot();
		...
		if (!CTFEvent.onAction(player, getObjectId()) || !DMEvent.onAction(player, getObjectId())
				|| !LMEvent.onAction(player, getObjectId()) || !TvTEvent.onAction(player, getObjectId()))
			return;
	...
	/**
	 * Check if this {@link Player} can open any type of private shop (buy, sell,
	 * manufacture). Show him the error message, if any.
	 * 
	...
	public boolean canOpenPrivateStore(boolean cancelActiveTrade) {
		...
		if (isInDuel() || AttackStanceTaskManager.getInstance().isInAttackStance(this) || getPvpFlag() > 0
				|| getAttack().isAttackingNow()) {
		...
		// Can't open a private store if already sat (isInStoreMode() is checked in
		// order toggle off works).
	...
	public void tryOpenPrivateSellStore(boolean isPackageSale) {
		...
		if (getOperateType() == OperateType.NONE || getOperateType() == OperateType.SELL
	...
	public void setTarget(WorldObject newTarget) {
		...
		// Add the Player to the _statusListener of the new target if it's a Creature
			...
			sendPacket(new MyTargetSelected(target.getObjectId(),
					(target.isAttackableBy(this) || target instanceof Summon)
							? getStatus().getLevel() - target.getStatus().getLevel()
							: 0));
			...
			if (isCursedWeaponEquipped())
				...
				if (pk == null || !pk.isCursedWeaponEquipped()) {
					...
					if (Config.ALLOW_DELEVEL && (!hasSkill(L2Skill.SKILL_LUCKY) || getStatus().getLevel() > 9))
						applyDeathPenalty(pk != null && getClan() != null && pk.getClan() != null
	...
	public void onKillUpdatePvPKarma(Playable target) {
		...
		if (checkIfPvP(target) || (targetPlayer.getClan() != null && getClan() != null
			...
			if (target instanceof Player && AntiFeedManager.getInstance().check(this, target)) {
				...
				if (Config.ANNOUNCE_KILL)
					World.toAllOnlinePlayers(SystemMessage
							.getSystemMessage(SystemMessageId.S1_S2).addString(Config.ANNOUNCE_PVP_MSG
									.replace("$killer", getName()).replace("$target", targetPlayer.getName()))
							.addZoneName(targetPlayer.getPosition()));
		...
		// Otherwise, killer is considered as a PKer.
			...
			if (Config.ANNOUNCE_KILL)
				World.toAllOnlinePlayers(SystemMessage
						.getSystemMessage(SystemMessageId.S1_S2).addString(Config.ANNOUNCE_PK_MSG
								.replace("$killer", getName()).replace("$target", targetPlayer.getName()))
						.addZoneName(targetPlayer.getPosition()));
	 ...
	 *                         regular system.
	 * @param killedByPlayable : If true, we use specific rules if killed by
	 *                         playable only.
	 */
	public void applyDeathPenalty(boolean atWar, boolean killedByPlayable) {
		...
		if (lvl < maxLevel)
			lostExp = Math
					.round((getStatus().getExpForLevel(lvl + 1) - getStatus().getExpForLevel(lvl)) * percentLost / 100);
		else
			lostExp = Math.round((getStatus().getExpForLevel(maxLevel) - getStatus().getExpForLevel(maxLevel - 1))
					* percentLost / 100);
	...
	/**
	 * Remove the {@link Player} from both waiting list and any potential
	 * {@link PartyMatchRoom}.
	 */
	...
	/**
	 * Set the request expire time of that {@link Player}, and set his
	 * {@link Player} partner as the active requester.
	 * 
	...
	public void setOperateType(OperateType type) {
		...
		if (Config.OFFLINE_DISCONNECT_FINISHED && type == OperateType.NONE
				&& ((getClient() == null) || getClient().isDetached()))
			logout(true);
	}

	/**
	 * @return True if this {@link Player} is set on any store mode, or false
	 *         otherwise.
	 */
	public boolean isInStoreMode() {
		return _operateType == OperateType.BUY || _operateType == OperateType.SELL
				|| _operateType == OperateType.PACKAGE_SELL || _operateType == OperateType.MANUFACTURE;
	}

	/**
	 * @return True if this {@link Player} is set on any store manage mode, or false
	 *         otherwise.
	 */
	public boolean isInManageStoreMode() {
		return _operateType == OperateType.BUY_MANAGE || _operateType == OperateType.SELL_MANAGE
				|| _operateType == OperateType.MANUFACTURE_MANAGE;
	}
	...
	private void removeInvalidSkills() {
		...
		final Map<Integer, Optional<GeneralSkillNode>> availableSkills = getTemplate().getSkills().stream()
				.filter(s -> s.getMinLvl() <= getStatus().getLevel() + ((s.getId() == L2Skill.SKILL_EXPERTISE) ? 0 : 9))
		...
		for (final L2Skill skill : getSkills().values()) {
			...
			if (skill.getLevel() > maxLevel) {
				...
				if ((getStatus().getLevel() < 76 || availableSkill.getValue() < maxLevel)
	...
	public List<GeneralSkillNode> getAvailableAutoGetSkills() {
		...
		getTemplate().getSkills().stream().filter(s -> s.getMinLvl() <= getStatus().getLevel() && s.getCost() == 0)
				.collect(Collectors.groupingBy(s -> s.getId(), Collectors.maxBy(COMPARE_SKILLS_BY_LVL)))
				.forEach((i, s) -> {
	...
	public List<GeneralSkillNode> getAllAvailableSkills() {
		...
		getTemplate().getSkills().stream().filter(s -> s.getMinLvl() <= getStatus().getLevel())
				.collect(Collectors.groupingBy(s -> s.getId(), Collectors.maxBy(COMPARE_SKILLS_BY_LVL)))
				.forEach((i, s) -> {
					if (getSkillLevel(i) < s.get().getValue())
						result.add(s.get());
	...
	public int getRequiredLevelForNextSkill() {
		return getTemplate().getSkills().stream()
				.filter(s -> s.getMinLvl() > getStatus().getLevel() && s.getCost() != 0).min(COMPARE_SKILLS_BY_MIN_LVL)
				.map(s -> s.getMinLvl()).orElse(0);
	}
	 ...
	 * 
	 * @param leftHandIncluded : If set to True, the left hand item is also
	 *                         disarmed.
	...
	public boolean disarmWeapon(boolean leftHandIncluded) {
		...
		// If InventoryUpdate packet is filled, it means either weapon or sidearm has
		// been disarmed. An update is needed.
		if (iu != null) {
			sendPacket(iu);
			broadcastUserInfo();
		}
	...
	/**
	 * Test if this {@link Player} can mount the selected {@link Summon} or dismount
	 * if already mounted, and act accordingly.<br>
	 * <br>
	 * This method is used by both "Actions" panel "Mount/Dismount" button, and
	 * /mount /dismount usercommands.
	 * 
	...
	public void mountPlayer(Summon summon) {
		if (summon instanceof Pet && summon.isMountable() && !isMounted() && !isBetrayed()) {
			...
			if (getMountType() == 2) {
				...
				// Unsafe fall height.
				if (Math.abs(getZ() - GeoEngine.getInstance().getHeight(getPosition())) > getTemplate()
						.getSafeFallHeight(getAppearance().getSex())) {
	...
	protected class FeedTask implements Runnable {
		...
		public void run() {
			...
			if (getCurrentFeed() > getFeedConsume())
				...
				// If Player was wyvern mounted, then he is teleported to nearest town to avoid
				// the falling damage.
				if (wasFlying)
	...
	public void setAccessLevel(int level) {
		...
		if (level > 0)
			setTitle(accessLevel.getName()); // For level lower or equal to user, we don't apply AccessLevel name as
												// title.
	...
	private void restoreCharData() {
		...
		// Retrieve from the database all recom data of this Player and add them to
		// _recomChars.
		restoreRecom();

		// Retrieve from the database all quest states and variables for this Player and
		// add them to _quests.
		_questList.restore();
	}
	...
	private void restoreSkills() {
		try (Connection con = ConnectionPool.getConnection();
				PreparedStatement ps = con.prepareStatement(
						Config.SUBCLASS_SKILLS ? RESTORE_SKILLS_FOR_CHAR_ALT_SUBCLASS : RESTORE_SKILLS_FOR_CHAR)) {
			ps.setInt(1, getObjectId());
	...
	public boolean canCastBeneficialSkillOnPlayable(Playable target, L2Skill skill, boolean isCtrlPressed) {
		...
		// Check if the player is in the Arena event zone
		if (isInsideZone(ZoneId.ARENA_EVENT) && targetPlayer.isInsideZone(ZoneId.ARENA_EVENT)) {
			// Check if both players are in the same party
			if (isInParty() && targetPlayer.isInParty()
					&& getParty().getLeader() == targetPlayer.getParty().getLeader()) {
				// Allow skill use without holding Ctrl
				return true;
			}
		}
		...
		final boolean sameParty = (isInParty() && targetPlayer.isInParty()
				&& getParty().getLeader() == targetPlayer.getParty().getLeader());
		final boolean sameCommandChannel = (isInParty() && targetPlayer.isInParty()
		...
		// If the target not from the same CC/party/alliance/clan is flagged / PK, you
		// can buff with isCtrlPressed.
		if (targetPlayer.getPvpFlag() > 0 || targetPlayer.getKarma() > 0)
			return isCtrlPressed;

		// If the target not from the same CC/party/alliance/clan is white, it may be
		// freely buffed
		return true;
	}
	 ...
	 * <br>
	 * If the parameter is null but Player is registered into a Boat, we delete the
	...
	private synchronized void cleanup() {
		try {
			...
			getMemos().set("botRange", isbotRangeDOA());
	...
	/**
	 * Check and calculate if a new Death Penalty buff level needs to be added. If
	 * Death Penalty already applies, raise its level by 1.
	 * 
	...
	public void calculateDeathPenaltyBuffLevel(Creature killer) {
		...
		if ((getKarma() > 0 || Rnd.get(1, 100) <= Config.DEATH_PENALTY_CHANCE) && !(killer instanceof Player)
				&& !(isGM()) && !(getCharmOfLuck() && (killer == null || killer.isRaidRelated())) && !isPhoenixBlessed()
	...
	/**
	 * Reduce the Death Penalty buff effect from this {@link Player} of 1. If it
	 * reaches 0, remove it entirely.
	 */
	...
	public final void sendDamageMessage(Creature target, int damage, boolean mcrit, boolean pcrit, boolean miss) {
		...
		if (BossEvent.getInstance().getState() == BossEvent.EventState.FIGHTING && BossEvent.getInstance().eventPlayers.contains(this)
				&& target.getObjectId() == BossEvent.getInstance().objectId) {
	...
	public final boolean isFalling(int z) {
		...
		final int damage = (int) Formulas.calcFallDam(this, deltaZ,
				getBaseTemplate().getSafeFallHeight(getAppearance().getSex()));
		if (damage > 0) {
	...
	public double getCollisionRadius() {
		return (isMounted()) ? NpcData.getInstance().getTemplate(_mountNpcId).getCollisionRadius()
				: getBaseTemplate().getCollisionRadiusBySex(getAppearance().getSex());
	}

	@Override
	public double getCollisionHeight() {
		return (isMounted()) ? NpcData.getInstance().getTemplate(_mountNpcId).getCollisionHeight()
				: getBaseTemplate().getCollisionHeightBySex(getAppearance().getSex());
	}
	...
	public void activateGate(int answer, int type) {
		...
		if (answer == 1 && getTarget() == _requestedGate && _requestedGate.getClanHall() != null && getClan() != null
	...
	public void checkCondition(double curHp, double newHp) {
		...
		short[] _skills = { 290, 291, };

		// active skills.
		short[] _effectsSkillsId = { 292, 292 };

		byte[] _effectsHp = { 30, 60 };
		...
		for (int i = 0; i < _skills.length; i++) {
			if (getSkillLevel(_skills[i]) > 0) {
				if (_curHpPercent > _hp[i] && _newHpPercent <= _hp[i]) {
					sendPacket(SystemMessage.getSystemMessage(SystemMessageId.S1_HP_DECREASED_EFFECT_APPLIES)
							.addSkillName(_skills[i]));
					needsUpdate = true;
				} else if (_curHpPercent <= _hp[i] && _newHpPercent > _hp[i]) {
					sendPacket(SystemMessage.getSystemMessage(SystemMessageId.S1_HP_INCREASED_EFFECT_DISAPPEARS)
							.addSkillName(_skills[i]));
					needsUpdate = true;
				}
			}
		...
		for (int i = 0; i < _effectsSkillsId.length; i++) {
			if (getFirstEffect(_effectsSkillsId[i]) != null) {
				if (_curHpPercent > _effectsHp[i] && _newHpPercent <= _effectsHp[i]) {
					sendPacket(SystemMessage.getSystemMessage(SystemMessageId.S1_HP_DECREASED_EFFECT_APPLIES)
							.addSkillName(_effectsSkillsId[i]));
					needsUpdate = true;
	...
	public void saveTradeList() {
		...
		if (_sellList == null || _sellList.isEmpty())
			...
			for (TradeItem i : _sellList)
				tradeListBuilder.append(i.getObjectId()).append(",").append(i.getCount()).append(",")
	...
	public boolean setbot(boolean b) {
		return _isbot = b;
	}

	private final ExclusiveBot _bot = new ExclusiveBot(this);

	public ExclusiveBot getBot() {
		return _bot;
	}

	private boolean _isPotionDOA;

	public boolean isPotionDOA() {
		return _isPotionDOA;
	}

	public boolean setPotionDOA(boolean b) {
		return _isPotionDOA = b;
	}

	private boolean _isPetAttack;

	public boolean isPetAttackDOA() {
		return _isPetAttack;
	}

	public boolean setPetAttackDOA(boolean b) {
		return _isPetAttack = b;
	}

	private boolean _isPartyAssist;

	public boolean isPartyAssistDOA() {
		return _isPartyAssist;
	}

	public boolean setPartyAssistDOA(boolean b) {
		return _isPartyAssist = b;
	}

	private int _isbotRange;

	public int isbotRangeDOA() {
		return _isbotRange;

	}

	public int setbotRangeDOA(int b) {
		return _isbotRange = b;
	}

	private boolean _isForceAttack;

	public boolean isForceAttackDOA() {
		return _isForceAttack;
	}

	public boolean setForceAttackDOA(boolean b) {
		return _isForceAttack = b;
	}

	private boolean _isAutoBuffer;

	public boolean isAutoBufferDOA() {
		return _isAutoBuffer;
	}

	public boolean setAutoBufferDOA(boolean b) {
		return _isAutoBuffer = b;
	}

	private boolean _isSelectBuffer;

	public boolean isSelectBufferDOA() {
		return _isSelectBuffer;
	}

	public boolean setSelectBufferDOA(boolean b) {
		return _isSelectBuffer = b;
	}

	private boolean _isViwerRandios;

	public boolean isViwerRandiosDOA() {
		return _isViwerRandios;
	}

	public boolean setViwerRandiosDOA(boolean b) {
		return _isViwerRandios = b;
	}

	private Map<Integer, L2Skill> activeSkills;

	public void setSkills(Map<Integer, L2Skill> skills) {
		this.activeSkills = new ConcurrentHashMap<>();
	...
	public Map<Integer, L2Skill> getActiveSkills() {
		return activeSkills;
	}

	public Map<Integer, L2Skill> getSkillsFromDataSource(Player fake, int classIndex) {
		...
		try (Connection con = ConnectionPool.getConnection();
			...
			try (ResultSet rs = ps.executeQuery()) {
				while (rs.next()) {
					...
					// Skip skills with restore type > 0
					if (restoreType > 0) {
						continue;
					}

					L2Skill skill = SkillTable.getInstance().getInfo(skillId, skillLevel);
					if (skill != null && skill.isAutofarm()) {
						skills.put(skillId, skill);
					}
				}
			}
		} catch (SQLException e) {
			LOGGER.error("Couldn't retrieve skills from data source.", e);
		}

		return skills;
	}

	private boolean _playerUseAgathion = false;

	public boolean getPlayerAgathion() {
		return _playerUseAgathion;
	}

	public void setPlayerAgathion(boolean j) {
		_playerUseAgathion = j;
	}

	private boolean _playerAgathionPickUp = false;

	public boolean getAgathionPickUp() {
		return _playerAgathionPickUp;
	}

	public void setAgathionPickUp(boolean item) {
		_playerAgathionPickUp = item;
	}

	private boolean _adenaForGb = false;

	public boolean isAutoAdena() {
		return _adenaForGb;
	}

	public void setAutoAdena(boolean gbforadena) {
		_adenaForGb = gbforadena;
	}
	...
	public boolean isHairSkin() {
		return _hairSkin.get();
	}

	public void setHairSkin(boolean value) {
		_hairSkin.set(value);
	}

	public boolean getHairSkin() {
		try (Connection con = ConnectionPool.getConnection();
				PreparedStatement statement = con.prepareStatement("SELECT hair_skin from characters WHERE obj_Id=?")) {
			statement.setInt(1, getObjectId());
			try (ResultSet rset = statement.executeQuery()) {
				while (rset.next())
					_id_hair_skin = rset.getInt("hair_skin");
			}
		} catch (Exception e) {
			LOGGER.warn("Skin error: " + e);
		}
	...
	public String getSkin() {
		try (Connection con = ConnectionPool.getConnection();
				PreparedStatement statement = con.prepareStatement("SELECT skin from characters WHERE obj_Id=?")) {
			statement.setInt(1, getObjectId());
			try (ResultSet rset = statement.executeQuery()) {
				while (rset.next())
					_id_skin = rset.getInt("skin");
			}
		} catch (Exception e) {
			LOGGER.warn("Skin error: " + e);
		}
		...
		if (_id_skin > 0)
			skin_name = getNameSkin(_id_skin);
		return skin_name;
	}

	public static String getNameSkin(int IdSkin) {
		String Name = null;
		...
		case 30029:
			...
			break;
		case 30031:
			Name = "Transcendent Eternal Robe";
			break;
		case 30032:
			Name = "Transcendent Eternal Light";
			break;
		case 30033:
			Name = "Transcendent Eternal Heavy";
			break;
		case 30034:
			Name = "Agent Chaos";
			break;
