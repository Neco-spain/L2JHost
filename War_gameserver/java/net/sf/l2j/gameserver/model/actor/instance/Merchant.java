...
import net.sf.l2j.gameserver.data.manager.CustomBuyListManager;
...
public class Merchant extends Folk
{
	...
	public void onBypassFeedback(Player player, String command)
	{
		...
		if (actualCommand.equalsIgnoreCase("CustomBuyList"))
		{
			if (st.countTokens() < 1)
				return;
			
			showCustomBuyWindow(player, Integer.parseInt(st.nextToken()));
		}
	...
	private void showCustomBuyWindow(Player player, int val)
	{
		final NpcBuyList buyList = CustomBuyListManager.getInstance().getBuyListCustom(val);
		if (buyList == null || !buyList.isNpcAllowed(getNpcId()))
			return;
