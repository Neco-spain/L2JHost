...
public abstract class Playable extends Creature
{
	...
	public boolean isAttackableBy(Creature attacker)
	{
		...
		if (attacker instanceof Playable)
		{
			...
			if (isInsideZone(ZoneId.PVP))
			  ...
			  // Check if the player is in the Arena event zone
		    if (isInsideZone(ZoneId.ARENA_EVENT) && attackerPlayable.isInsideZone(ZoneId.ARENA_EVENT)) {
		        // Check if both players are in the same party
		        if (isInParty() && attackerPlayable.isInParty() && getParty().getLeader() == attackerPlayable.getParty().getLeader()) {
		            // Allow skill use without holding Ctrl
		            return true;
		        }
		    }
	...
	public boolean isAttackableWithoutForceBy(Playable attacker)
	{
		...
		if (CTFEvent.isStarted() || TvTEvent.isStarted() || LMEvent.isStarted() || DMEvent.isStarted() && getActingPlayer().getTeam() == attacker.getActingPlayer().getTeam())
		  ...
		  // Check if the player is in the Arena event zone
	    if (isInsideZone(ZoneId.ARENA_EVENT) && attackerPlayer.isInsideZone(ZoneId.ARENA_EVENT)) {
	        // Check if both players are in the same party
	        if (isInParty() && attackerPlayer.isInParty() && getParty().getLeader() == attackerPlayer.getParty().getLeader()) {
	            // Allow skill use without holding Ctrl
	            return true;
	        }
	    }
	...
	public boolean canKeepAttacking(Creature target)
	{
		...
		if (target instanceof Playable)
		{
			...
			if (isInsideZone(ZoneId.PVP) && target.isInsideZone(ZoneId.PVP))
			  ...
			  // Check if the player is in the Arena event zone
		    if (isInsideZone(ZoneId.ARENA_EVENT) && target.isInsideZone(ZoneId.ARENA_EVENT)) {
		        // Check if both players are in the same party
		        if (isInParty() && target.isInParty() && getParty().getLeader() == target.getParty().getLeader()) {
		            // Allow skill use without holding Ctrl
		            return true;
		        }
		    }
