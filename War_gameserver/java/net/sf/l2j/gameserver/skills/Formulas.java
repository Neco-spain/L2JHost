...
public final class Formulas
{
	...
	public static final double calcPhysicalAttackDamage(Creature attacker, Creature target, ShieldDefense sDef, boolean crit, boolean ss)
	{
		...
		{
			if (attacker instanceof Player && target instanceof Player && attacker != target)
			{
				Player at = (Player) attacker;
				Player t = (Player) target;
				
				double classToClassMod = ((BalanceManagerAI.getInstance().getPowerMod(at.getClassId().getId(), t.getClassId().getId()) / 100f) * damage);
				double classToAllMod = ((BalanceManagerAI.getInstance().getPowerMod(at.getClassId().getId(), -1) / 100f) * damage);
				damage += classToClassMod + classToAllMod;
				
				if (Config.ENABLE_BALANCE_LOGGER_PHYSICAL)
				{
					...
					LOGGER.info("Final balance damage: {}", damage);
				}
